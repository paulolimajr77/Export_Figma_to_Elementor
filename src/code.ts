...
// Define nodes that typically have geometry properties (fills, strokes, effects)
type GeometryNode =
    | RectangleNode
    | EllipseNode
    | PolygonNode
...
// Extract Typography
function extractTypography(node: SceneNode): ElementorSettings {
    const typo: ElementorSettings = {};
    ...
    if (node.textAlignHorizontal) {
        ...
        if (alignMap[node.textAlignHorizontal]) typo.align = alignMap[node.textAlignHorizontal];
    }
...
// Extract Text Color
function extractTextColor(node: SceneNode): string {
...
function extractBorderStyles(node: SceneNode): ElementorSettings {
    const border: ElementorSettings = {};
...
function extractShadows(node: SceneNode): ElementorSettings {
    if (!hasEffects(node) || !Array.isArray(node.effects) || node.effects.length === 0) return {};
...
// Extract Text Shadow
function extractTextShadow(node: SceneNode): ElementorSettings {
    if (node.type !== 'TEXT') return {};
    if (!hasEffects(node) || node.effects.length === 0) return {};

    const textShadows: any[] = [];
    node.effects.forEach(effect => {
        if (effect.type === 'DROP_SHADOW' && effect.visible !== false) {
...
function extractTransform(node: SceneNode): ElementorSettings {
    const transform: ElementorSettings = {};
...
function extractPositioning(node: SceneNode): ElementorSettings {
    const positioning: ElementorSettings = {};
...
function extractPadding(node: SceneNode): ElementorSettings {
    ...
    if (frame.paddingTop === 0 && frame.paddingLeft === 0 && frame.paddingRight === 0 && frame.paddingBottom === 0) return {};
...
function extractCustomCSS(node: SceneNode): ElementorSettings {
    ...
    if (node.type === 'VECTOR' || node.name.toLowerCase().includes('icon')) {
        if (hasFills(node) && Array.isArray(node.fills) && node.fills.length > 0 && node.fills[0].type === 'SOLID') {
            const iconColor = convertColor(node.fills[0] as SolidPaint);
...
class ElementorCompiler {
    config: any;
    widgetPresets: { [key: string]: { widgetType: string, settings: any } };
    ...
    compile(nodes: readonly SceneNode[]): ElementorTemplate {
        const elements = nodes.filter(isVisible).map(node => this.processNode(node)).filter(Boolean) as ElementorElement[];
    ...
    processNode(node: SceneNode): ElementorElement | null {
        ...
        if (name.indexOf('w:') !== -1) {
            ...
            if (match && match[1]) {
                ...
                if (widgetSlug === 'container' && hasLayout(node)) {
                    return this.createContainer(node as FrameNode);
                }
        ...
        if (isLarge) {
            if (['FRAME', 'GROUP', 'INSTANCE', 'COMPONENT'].includes(node.type)) {
                if (hasLayout(node)) return this.createContainer(node as FrameNode);
                // Fallback for Groups (treat as simple container if needed)
                if ('children' in node) return this.createContainer(node as unknown as FrameNode);
            }
            return null;
        }
        ...
        if (['FRAME', 'GROUP', 'INSTANCE'].includes(node.type)) {
            const composite = this.detectCompositePattern(node as FrameNode);
        ...
        if (hasFills(node) && this.hasImageFill(node)) {
            ...
            if ('children' in node && node.children.length > 0) return this.createContainer(node as FrameNode);
            return this.createExplicitWidget(node, 'image');
        }

        if (this.isIconNode(node)) return this.createExplicitWidget(node, 'icon');
        if (node.type === 'TEXT') return this.createTextWidget(node as TextNode);

        // Default Container
        if (['FRAME', 'GROUP', 'INSTANCE', 'COMPONENT'].includes(node.type)) {
            if ('children' in node) return this.createContainer(node as FrameNode);
        }

        return null;
    }

    createContainer(node: FrameNode): ElementorElement {
        ...
        const children = node.children.filter(isVisible).map(child => this.processNode(child)).filter(Boolean) as ElementorElement[];
    ...
    createExplicitWidget(node: SceneNode, widgetSlug: string): ElementorElement {
        ...
        if ('children' in node) {
            ...
            if (node.type === 'TEXT') textChildren.push(node as TextNode);
        }
        ...
        if (textChildren.length > 0) {
            ...
            if (settings.title === undefined && settings.title_text === undefined && settings.heading === undefined) {
                ...
                if (settings.title !== undefined) settings.title = titleNode.characters;
                if (settings.title_text !== undefined) settings.title_text = titleNode.characters;
                if (settings.heading !== undefined) settings.heading = titleNode.characters;
            }
        ...
        if (textChildren.length > 1) {
            ...
            if (settings.description === undefined && settings.description_text === undefined) {
                ...
                if (settings.description !== undefined) settings.description = descNode.characters;
                if (settings.description_text !== undefined) settings.description_text = descNode.characters;
            }
        ...
        // 3. Map Icon
        if (iconChild) {
            let iconFill: Paint | null = null;
            // FIX: Check 'fills' existence explicitly and cast to avoid 'never' type inference
            if (hasFills(iconChild)) {
                const geoNode = iconChild as GeometryNode;
                if (Array.isArray(geoNode.fills) && geoNode.fills.length > 0) {
                    iconFill = geoNode.fills[0];
                }
            }
            else if ('children' in iconChild) {
                const cFrame = iconChild as FrameNode;
                const vectorChild = cFrame.children.find((c: SceneNode) => hasFills(c) && Array.isArray(c.fills) && c.fills.length > 0) as GeometryNode;
                if (vectorChild) iconFill = vectorChild.fills[0];
            }
    ...
    detectCompositePattern(node: FrameNode): ElementorElement | null {
        ...
        for (const child of sortedChildren) {
            ...
            else if (hasFills(child) && this.hasImageFill(child as GeometryNode)) hasImage = true;
    ...
    isIconNode(node: SceneNode): boolean {
        const vectorTypes: string[] = ['VECTOR', 'STAR', 'ELLIPSE', 'POLYGON', 'BOOLEAN_OPERATION', 'LINE'];
    ...
    hasImageFill(node: GeometryNode): boolean {
        return 'fills' in node && Array.isArray(node.fills) && node.fills.some((p: Paint) => p.type === 'IMAGE');
    }

    createTextWidget(node: TextNode): ElementorElement {
    ...
    debugNodeRecursive(node: SceneNode, depth: number): any {
        ...
        if (node.type === 'TEXT') {
            info.extractedStyles.typography = extractTypography(node as TextNode);
        }
