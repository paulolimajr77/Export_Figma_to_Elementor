import type { RelativePosition } from '../types/elementor.types';
 ...
 */
export function detectRelativePosition(source: SceneNode, target: SceneNode): RelativePosition {
 ...
 */
export function calculateDistance(node1: SceneNode, node2: SceneNode): number {
    if (!node1.absoluteBoundingBox || !node2.absoluteBoundingBox) return Infinity;
 ...
 */
export function isNodeInside(child: SceneNode, parent: SceneNode): boolean {
    if (!child.absoluteBoundingBox || !parent.absoluteBoundingBox) return false;
