 ...
 */
export function isExternalContainer(node: SceneNode, isTopLevel: boolean = false): boolean {
 ...
 */
export function isInnerContainer(node: SceneNode, parentNode: SceneNode | null): boolean {
    if (!hasLayout(node)) return false;
