...
function node(overrides: Partial<SerializedNode> = {}): SerializedNode {
...
function asSchema(root: SerializedNode) {
  return convertToFlexSchema(analyzeTreeWithHeuristics(root));
}

describe('NO-AI heuristics', () => {
  ...
  it('detecta heading', () => {
    ...
    const w = asSchema(root).containers[0].widgets[0];
  ...
  it('colapsa frame com imagens aninhadas em um widget de imagem', () => {
    ...
    const root = node({ children: [wrapper] });
    const w = asSchema(root).containers[0].widgets[0];
    expect(w.type).toBe('image');
    expect(w.imageId).toBe('img-nested');
  });

  it('detecta button pelo nome', () => {
  ...
  it('detecta icon-box', () => {
    ...
    const root = node({ children: [wrapper] });
    const w = asSchema(root).containers[0].widgets[0];
    expect(w.type).toBe('icon_box');
    expect(w.content).toBe('Descricao');
  });

  it('detecta basic-gallery', () => {
    ...
    const root = node({ children: [wrapper] });
    const w = asSchema(root).containers[0].widgets[0];
    expect(w.type).toBe('basic-gallery');
  });

  it('detecta icon-list', () => {
    ...
    const root = node({ children: [wrapper] });
    const w = asSchema(root).containers[0].widgets[0];
    expect(w.type).toBe('icon_list');
  });

  it('fallback w:custom quando não reconhece', () => {
    const child = node({ id: 'c1', type: 'RECTANGLE', name: 'shape' });
    const root = node({ children: [child] });
    const w = asSchema(root).containers[0].widgets[0];
    expect(w.type).toBe('custom');
    expect(w.content).toBe('shape');
  });
